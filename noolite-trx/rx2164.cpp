...
void RX2164::_handleEvent(int new_togl, unsigned char *input)
{
    ...
    traceNotice("TOGL: %u \r\n", new_togl);
    traceNotice("Action: %s[%u]\r\n", _actionToStr(action).c_str(), action);
    traceNotice("Channel: %u\r\n", channel);
    traceNotice("Data Size: %u\r\n", _formatToDataSize(data_format));
    ...
    _callback_func(new_togl, action, channel, data_format, input[4], input[5], input[6], input[7]);
    ...
    if(it_channel != _channel_actions.end())
    {
        ...
        {
            string command = it_action->second.script + " --ch " + std::to_string(channel) + " --act " + std::to_string(action);

            if(it_action->second.fw)
            {
                command += " --togl " + std::to_string(new_togl) + " --df " + std::to_string(data_format);
            }
